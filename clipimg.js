$.clip = {
    ...
    initEvent: function () {
        ...
        $("#btnCancel").off().on("click", function () {
            $("#clipImg").hide();
            $("#idPersonFile").parent().html('<input style="display:none;" type="file" id="idPersonFile" name="idPersonFile" />选择');
            $("#clipImg").html("");
            $.clip.initEvent()
        });
    },
    ...
    init: function () {
        ...
        $canvas[0].width = c_w * 0.70;
        $canvas[0].height = parseInt($canvas[0].width * 600 / 450);
    ...
    clipImg: function () {
        ...
        try {
            $.post("/UploadFile/UploadBase64Img", { file: base64 }, function (res) {
                if (res.Code == 0) {
                    $("#imgPerson").attr("src", res.Data.url) + "?t=" + new Date().getTime();
                    $("#imgurl").val(res.Data.url)
                    var parent = $("#imgPerson").parent();
